...
from utils.PathUtils import root_path


class RsaUtil(object):
    # 使用 rsa库进行RSA签名和加解密

    PUBLIC_KEY_PATH = (root_path+'/common/public.rsa')  # 公钥
    # 公钥'./rsa/private.rsa'  # 私钥
    PRIVATE_KEY_PATH = (root_path+'/common/private.rsa')
    ...
    def __init__(self,
        ...
        if company_pub_file:
            self.company_public_key = rsa.PublicKey.load_pkcs1_openssl_pem(
    ...
    def encrypt_by_public_key(self, message):
        """使用公钥加密.
            ...
            加密之后需要对接过进行base64转码
        """
        encrypt_result = b''  # 这里进行了修改 先前b'' 现在修改喂‘’
    ...
    def sign_by_private_key(self, data):
        """私钥签名.
            ...
            签名之后，需要转义后输出
        """
        # data=str(data1, encoding = "utf8")
        # signature=rsa.sign(str(data), priv_key=self.company_private_key,hash_method= 'SHA-1')
        # 这里吧hash 去掉 改为了hash_method
        signature = rsa.sign(
            data, priv_key=self.company_private_key, hash_method='SHA-1')
